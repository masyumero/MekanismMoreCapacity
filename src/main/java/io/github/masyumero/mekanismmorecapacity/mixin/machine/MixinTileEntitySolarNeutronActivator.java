...
public abstract class MixinTileEntitySolarNeutronActivator extends TileEntityRecipeMachine<ChemicalToChemicalRecipe> implements IBoundingBlock, ISingleRecipeLookupHandler.ChemicalRecipeLookupHandler<ChemicalToChemicalRecipe> {
    ...
    protected MixinTileEntitySolarNeutronActivator(Holder<Block> blockProvider, BlockPos pos, BlockState state, List<CachedRecipe.OperationTracker.RecipeError> errorTypes) {
    ...
    public IChemicalTankHolder getInitialChemicalTanks(ChemicalTankHelper instance, IContentsListener listener, IContentsListener recipeCacheListener, IContentsListener recipeCacheUnpauseListener) {
        ...
        builder.addTank(inputTank = BasicChemicalTank.createModern(MMCConfig.MEK_MACHINE_CONFIG.SolarNeutronActivatorInput.get(), ChemicalTankHelper.radioactiveInputTankPredicate(() -> outputTank),
                ConstantPredicates.alwaysTrueBi(), this::containsRecipe, ChemicalAttributeValidator.ALWAYS_ALLOW, recipeCacheListener));
        builder.addTank(outputTank = BasicChemicalTank.output(MMCConfig.MEK_MACHINE_CONFIG.SolarNeutronActivatorOutput.get(), recipeCacheUnpauseListener));
